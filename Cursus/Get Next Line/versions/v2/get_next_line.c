...
/*   Updated: 2022/10/06 16:28:28 by angmarti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ssize_t	gnl_read_and_add(int fd, char **currentline, size_t len)
{
	...
	while (i < len + result)
		*(newline + i++) = *(buffer + j++);
	*(newline + i) = '\0';
	gnl_replace_line(currentline, newline);
	//free(buffer);
	// *currentline = ft_strjoin(*currentline, buffer);
	return (result);
}
	// free(newline);
...
char	*get_next_line(int fd)
{
	...
	while (result != -1)
	{
		...
		while (line && line[len])
			if (line[len++] == '\n')
				break ;
		if (line && line[0] && line[len - 1] == '\n')
			return (gnl_cut_line(&line, len));
		if (line && *line && !result)
			return (gnl_cut_line(&line, len));
		if (!result)
			return (NULL);
		result = gnl_read_and_add(fd, &line, len);
	}
	return (NULL);
}
